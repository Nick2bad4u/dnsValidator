...
describe("validators suggestions extra coverage", () => {
    ...
    const cases: [any, RegExp][] = [
        ...
        [
            { type: "SRV", name: "example.com", priority: -1 },
            /SRV records require/,
        ],
        ...
        [
            { type: "NSEC", nextDomainName: "bad..domain" },
            /NSEC records require/,
        ],
    ...
    it.each(cases)("invalid %p record suggestions", (rec, expectMsg) => {
        const res = validateDNSRecord(rec);

        expect(res.isValid).toBeFalsy();
        expect(res.errors.some((e) => expectMsg.test(e))).toBeTruthy();
    });
});
