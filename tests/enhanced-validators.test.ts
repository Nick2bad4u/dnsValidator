...
describe("enhanced Validators", () => {
    describe(validateARecord, () => {
        ...
        it("should reject wrong record type", () => {
            ...
            expect(result.isValid).toBeFalsy();
            expect(result.errors).toContain(
                "Expected record type 'A', got 'AAAA'"
            );
        });
    });
    ...
    describe("edge Cases", () => {
        it("should handle null and undefined inputs", () => {
            expect(validateARecord(null).isValid).toBeFalsy();
            expect(validateARecord(undefined).isValid).toBeFalsy();
            expect(validateAAAARecord(null).isValid).toBeFalsy();
            expect(validateMXRecord(undefined).isValid).toBeFalsy();
        });

        it("should handle non-object inputs", () => {
            expect(validateARecord("string").isValid).toBeFalsy();
            expect(validateAAAARecord(123).isValid).toBeFalsy();
            expect(validateMXRecord([]).isValid).toBeFalsy();
        });
