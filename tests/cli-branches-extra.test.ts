...
describe("command line extra branches", () => {
    ...
    it("record strict invalid structure exits with error", () => {
    ...
    it("record strict unsupported type exits with invalid structure error (falls through basic validation first)", () => {
        const data = JSON.stringify({});
        const res = captureRun([
            "record",
            "--data",
    ...
    it("record csv output formatting and quote escaping", () => {
    ...
    it("record table output truncation", () => {
    ...
    it("query path missing input triggers error exit", () => {
    ...
    it("bulk missing file option exits", () => {
    ...
    it("bulk queries mode with mixed success still exercises summary reporting branch", () => {
    ...
    it("bulk records output to file path", () => {
        const file = "tmp-bulk-output.json";
        const out = "tmp-bulk-results.txt";
        fs.writeFileSync(
            file,
            JSON.stringify([{ type: "A", address: "1.2.3.4" }])
        );
        const res = captureRun([
            "bulk",
            "--file",
